...
class TradingBot:
    ...
    def __init__(self, platform: str = "kalshi"):
        ...
        self.order_executor = OrderExecutor(client=self.client, config=self.config)
        ...
        self.risk_manager = RiskManager(
            client=self.client, config=self.config, fee_calculator=self.fee_calculator
        )
    ...
    async def initialize(self):
        ...
        try:
            ...
            self.logger.info(
                f"Environment: {'DEMO' if self.config.KALSHI_DEMO else 'PRODUCTION'}"
            )
            ...
            markets = await self.client.get_markets(
                status="open", filter_untradeable=False
            )
    ...
    async def market_data_callback(self, market_update: Market):
        """Callback function to process real-time market data from WebSocket."""

        try:

            # Update price history

            self.strategy_manager.on_market_update(market_update)

            # Persist updates to SQLite

            self.db.save_markets([market_update])

            self.logger.debug(f"Updated price history for {market_update.market_id}")

            # Generate signals

            signals = self.strategy_manager.generate_entry_signals([market_update])
            ...
            if signals:
                ...
                for signal in signals:
                    ...
                    # Get full market data from cache or fetch it

                    full_market = await self.get_full_market_data(signal.market_id)

                    if not full_market:

                        self.logger.warning(
                            f"Could not retrieve full market data for {signal.market_id}"
                        )
                    ...
                    if await self.should_trade_signal(full_market, signal):

                        await self.execute_signal_trade(signal, full_market)

            else:

                self.logger.debug(
                    f"No signals generated from market update {market_update.market_id}"
                )

        except Exception as e:

            self.logger.error(f"Error in market_data_callback: {e}", exc_info=True)

    async def get_full_market_data(self, market_id: str) -> Optional[Market]:
        """Get full market data from cache or fetch via REST API."""
        ...
        if market_id in self.market_cache:
            ...
            cached_data = self.market_cache[market_id]

            if now - cached_data["timestamp"] < timedelta(minutes=5):

                return cached_data["market"]

        # Fetch from API if not in cache or expired

        try:

            self.logger.info(f"Fetching full market data for {market_id}")

            market = await self.client.get_market(market_id)

            if market:

                self.market_cache[market_id] = {"timestamp": now, "market": market}

                return market

        except Exception as e:

            self.logger.error(f"Failed to fetch full market data for {market_id}: {e}")
    ...
    async def should_trade_signal(self, market, signal) -> bool:
        ...
        spike_obj = MockSpike(change_pct=magnitude, market_id=signal.market_id)
        ...
        corr_passed, corr_reason = self.correlation_manager.check_exposure(
            market.market_id, estimated_cost
        )
            ...
            try:
                ...
                for position in positions:
                    ...
                    exit_decision = self.position_manager.evaluate_position(
                        position_id=position.id, current_price=position.current_price
                    )

                    if exit_decision.should_exit:

                        await self._execute_exit(
                            position=position, reason=exit_decision.reason
                        )
                    ...
                    try:
                        ...
                        exit_signals = self.strategy_manager.generate_exit_signals(
                            [position], market_map
                        )
        ...
        try:
            ...
            if order and order.get("success"):
                ...
                if order_details and hasattr(order_details, "order_id"):
                    ...
                    self.logger.warning(
                        f"⚠️ Order submission did not return order_id for {spike.market_id}"
                    )
            ...
            self.logger.error(
                f"❌ Trade execution failed for {spike.market_id}: {e}", exc_info=True
            )
    ...
    async def _execute_exit(self, position, reason):
        ...
        try:
            ...
            self.logger.info(f"Exiting position {position.id} (reason: {reason})")
            ...
            if exit_order_result and exit_order_result.get("success"):
                ...
                pnl = self.position_manager.calculate_pnl(
                    position=position,
                    exit_price=(
                        exit_order.avg_fill_price
                        if hasattr(exit_order, "avg_fill_price")
                ...
                if self.starting_balance > 0:
                    ...
                    max_loss_usd = (
                        self.starting_balance * self.config.MAX_DAILY_LOSS_PCT
                    )

                    if (
                        self.daily_pnl <= -(max_loss_usd * 0.8)
                ...
                self.logger.error(
                    f"Exit order submission failed for position {position.id}"
                )
    ...
    async def run(self):
        ...
        try:
            ...
            await asyncio.gather(
                self.client.start_streaming(self.market_data_callback),
    ...
    async def shutdown(self):
        ...
        if hasattr(self.client, "stop_streaming"):
        ...
        try:
            ...
            for position in positions:
                ...
                try:
                    ...
                    exit_decision = self.position_manager.evaluate_position(
                        position_id=position["id"], current_price=current_price
                    )
