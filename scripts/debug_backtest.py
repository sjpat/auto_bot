...
for i, point in enumerate(prices):
    ...
    if i > 0:
        ...
        print(
            f"{i:3d}. {timestamp.strftime('%H:%M:%S')} - ${price:.3f} ({change:+.2f}%){spike_marker}"
        )
    else:
...
for i in range(1, len(prices)):
    ...
    if abs(change) >= 0.04:
        ...
        if entry_side == "no":
            ...
            print(
                f"   NO cost: {contracts} × (1.0 - ${entry_price:.3f}) = ${entry_cost:.2f}"
            )
        else:
        ...
        if entry_side == "no":
            ...
            print(
                f"   NO exit value: {contracts} × (1.0 - ${exit_price:.3f}) = ${exit_value:.2f}"
            )
        else:
            exit_value = contracts * exit_price
            print(
                f"   YES exit value: {contracts} × ${exit_price:.3f} = ${exit_value:.2f}"
            )
