...
async def main():
    ...
    try:
        ...
        filtered_markets = [
            m for m in open_markets if m.volume >= 5000
        ]  # $50+ in volume
        ...
        for i, market in enumerate(filtered_markets[:10], 1):  # Start with 10 markets
            ...
            try:
                history = await fetcher.fetch_market_history(
                    market_id=market.market_id, use_cache=True
                )
                ...
                if history and len(history) >= 20:  # Need at least 20 data points
                    ...
                    print(
                        f"      ✗ Insufficient history ({len(history) if history else 0} points)"
                    )
        ...
        if not historical_data:
            ...
            print(
                "   1. Run your monitor script for a few hours to build price history"
            )
        ...
        print(
            f"   Duration: {(end_date - start_date).total_seconds() / 3600:.1f} hours"
        )
        ...
        results = await engine.run_backtest(
            historical_data=historical_data, start_date=start_date, end_date=end_date
        )
