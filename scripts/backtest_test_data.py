...
async def main():
    ...
    print(
        f"   Trailing Stop: {backtest_config.USE_TRAILING_STOP} (Activate: ${backtest_config.TRAILING_STOP_ACTIVATION_USD}, Dist: ${backtest_config.TRAILING_STOP_DISTANCE_USD})"
    )
    ...
    print(
        f"   Total Return: ${results.total_return_usd:+,.2f} ({results.total_return_pct:+.2f}%)"
    )
    ...
    if results.rejection_reasons:
        ...
        for reason, count in sorted_reasons[:5]:
            pct = (
                (count / results.signals_rejected) * 100
                if results.signals_rejected > 0
                else 0
            )
    ...
    if results.trades:
        ...
        print(
            f"   {'Market':<20} {'Side':<6} {'Entry':<8} {'Exit':<8} {'P&L':<10} {'Strategy':<15} {'Reason':<20}"
        )
        print(f"   {'-'*95}")
        ...
        for i, trade in enumerate(results.trades[:10]):
            ...
            print(
                f"   {trade.market_id:<20} {side:<6} {entry:<8} {exit_price:<8} {pnl:<10} {strategy:<15} {reason:<20}"
            )
