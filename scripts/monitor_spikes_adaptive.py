        ...
        while True:
            ...
            for market in markets:
                ...
                spikes = spike_detector.detect_spikes(
                    markets=[market], threshold=threshold
                )
            ...
            if all_spikes:
                ...
                for spike in all_spikes:
                    ...
                    print(
                        f"  Change: {spike.change_pct:.2%} (threshold: {threshold_used:.1%})"
                    )
                ...
                markets_with_history = [
                    (
                        m.market_id,
                        len(spike_detector.price_history.get(m.market_id, [])),
                    )
                    for m in markets[:3]
                ]
                print(
                    f"  History depth: {[(mid[:20], depth) for mid, depth in markets_with_history]}"
                )
