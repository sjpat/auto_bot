...
def analyze_market_data(file_path):
    ...
    for market_id, points in markets.items():
        ...
        if isinstance(points[0], (int, float)):
            ...
            if all(abs((p.get('price', 0) + (1.0 - p.get('price', 0))) - 1.0) < 0.001 for p in points):
    ...
    print(f"Rejected (History):   {stats['insufficient_history']} (Too few data points)")
    print(f"Rejected (Liquidity): {stats['low_liquidity']} (Liquidity < ${MIN_LIQUIDITY})")
    print(f"Rejected (Spread):    {stats['wide_spread']} (Spread > {MAX_SPREAD:.0%})")
    print(f"Rejected (Too Slow):  {stats['slow_trends']} (Big moves, but < {SPIKE_THRESHOLD:.0%} per step)")
    print(f"Perfect Efficiency:   {stats['perfect_efficiency']} (Markets with 0 Arbitrage edge)")
    print("-" * 30)
    ...
    if stats['tradeable_candidates'] == 0:
        ...
        print("either too slow (gradual) for the Spike Strategy, or perfectly efficient")
