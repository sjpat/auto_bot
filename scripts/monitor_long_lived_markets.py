...
async def main():
    ...
    try:
        ...
        if len(long_lived_markets) == 0:
            ...
            print(
                "   2. Or use a different time of day when more markets are available"
            )
        ...
        long_lived_markets.sort(
            key=lambda m: (m.close_ts - now) + (m.liquidity_usd * 100), reverse=True
        )
        ...
        while True:
            ...
            # Fetch ALL markets
            all_markets = await client.get_markets(
                status="open", limit=300, min_volume=0, filter_untradeable=False
            )
                ...
                for mid in list(missing)[:3]:
                    if mid in market_details:
                        ...
                        if close_ts < now:
                            ...
                            print(
                                f"      - {mid[:50]}... ({hours_left:.1f}h remaining)"
                            )
            ...
            if markets_for_detection:
                ...
                if spikes:
                    ...
                    for spike in spikes:
                        ...
                        print(
                            f"  {spike.change_pct:.2%}: ${spike.previous_price:.4f} -> ${spike.current_price:.4f}"
                        )
                else:
                    print(
                ...
                if markets_ready == 0 and iteration > 20:
                    print(
                        f"   ⚠️  No markets ready after {iteration} checks - markets may be expiring"
                    )
