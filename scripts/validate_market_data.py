...
async def validate_market_data():
    ...
    try:
        ...
        markets = await client.get_markets(
            status="open", limit=50, min_volume=1, filter_untradeable=True
        )
        ...
        for i, market in enumerate(markets[:10], 1):
            ...
            if errors:
                validation_errors.append(
                    f"Market {market.market_id}: {', '.join(errors)}"
                )
        ...
        for market in markets[:5]:
            ...
            print(
                f"   Price: ${market.price:.4f} ({market.last_price_cents} basis points)"
            )
            print(
                f"   Bid: ${market.best_bid_cents/10000:.4f} | Ask: ${market.best_ask_cents/10000:.4f}"
            )
