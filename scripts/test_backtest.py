    ...
    try:
        ...
        if markets:
            ...
            history = await fetcher.fetch_market_history(
                market_id=test_market.market_id, use_cache=False
            )
            ...
            if history:
                print(
                    f"   First point: {history[0].timestamp} - ${history[0].price:.4f}"
                )
                print(
                    f"   Last point: {history[-1].timestamp} - ${history[-1].price:.4f}"
                )
        else:
