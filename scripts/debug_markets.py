...
async def main():
    ...
    print(
        f"API Key: {config.KALSHI_API_KEY[:5]}..."
    ...
    if config.KALSHI_DEMO:
        print(
            "⚠️  NOTE: You are in DEMO mode. Kalshi Demo often has very few or no open markets."
        )
    ...
    try:
        ...
        markets = await client.get_markets(
            status="open", limit=10, filter_untradeable=False
        )
        ...
        if not markets:
            print(
                "❌ No markets returned. This confirms the API is returning an empty list."
            )
        else:
            print(f"✅ Found {len(markets)} markets. First 3:")
            for m in markets[:3]:
                print(
                    f"   - {m.market_id} | Price: {m.price} | Liquidity: ${getattr(m, 'liquidity_usd', 0):.2f}"
                )
