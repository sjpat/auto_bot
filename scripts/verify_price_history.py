...
async def verify_price_history_building():
    ...
    try:
        ...
        markets = await client.get_markets(
            status="open", limit=20, min_volume=0, filter_untradeable=False
        )
        ...
        for i in range(30):
            ...
            spike_detector.add_price(test_market.market_id, simulated_price, timestamp)
        ...
        print(
            f"   - Time span: {(max(timestamps) - min(timestamps)).total_seconds() / 60:.1f} minutes"
        )
...
async def test_spike_detection_threshold():
    ...
    for i in range(25):
        spike_detector.add_price(
            market_id, base_price, base_time - timedelta(minutes=25 - i)
        )
    ...
    for test_price, description in test_cases:
        ...
        print(
            f"   ${test_price:.2f} ({description:20s}) | Change: {change_pct:+6.2f}% | {detected}"
        )
