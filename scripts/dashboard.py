...
def main():
    ...
    fig = px.line(
        df, x="trade_num", y="cumulative_pnl", title="Account Growth", markers=True
    )
    ...
    # Format table
    display_df = df[
        ["market_id", "side", "entry_price", "exit_price", "pnl", "exit_reason"]
    ].copy()
    display_df["pnl"] = display_df["pnl"].map("${:,.2f}".format)
    display_df["entry_price"] = display_df["entry_price"].map("${:,.4f}".format)

    st.dataframe(
        display_df.iloc[::-1].head(10), use_container_width=True
