    ...
    try:
        ...
        while True:
            ...
            markets = await client.get_markets(
                status="open", limit=200, min_volume=100, filter_untradeable=False
            )
            ...
            for i, market in enumerate(markets[:10], 1):  # Show first 10
                history_depth = len(
                    spike_detector.price_history.get(market.market_id, [])
                )
                vol_str = (
                    f"${market.liquidity_usd:.2f}" if market.liquidity_usd > 0 else "$0"
                )

                # Truncate ticker if too long
                ticker = (
                    market.market_id[:43] + "..."
                    if len(market.market_id) > 43
                    else market.market_id
                )

                print(
                    f"{i:<3} {ticker:<45} ${market.price:<9.4f} {vol_str:<8} {history_depth:<6}"
                )
            ...
            markets_ready = sum(
                1
                for mid in spike_detector.price_history
                if len(spike_detector.price_history[mid]) >= 20
            )
            total_tracked = len(spike_detector.price_history)
            print(
                f"   Markets ready for spike detection: {markets_ready}/{total_tracked}"
            )
            ...
            spikes = spike_detector.detect_spikes(
                markets=markets, threshold=config.SPIKE_THRESHOLD
            )
            ...
            if spikes:
                ...
                for i, spike in enumerate(spikes, 1):
                    ...
                    market = next(
                        (m for m in markets if m.market_id == spike.market_id), None
                    )
                    ...
                    print(
                        f"  Change: {spike.change_pct:.2%} (threshold: {config.SPIKE_THRESHOLD:.1%})"
                    )
                    ...
                    # Show history depth
                    history_depth = len(
                        spike_detector.price_history.get(spike.market_id, [])
                    )
            ...
            if markets_with_history and not spikes:
                ...
                for market in markets_with_history[:5]:
                    ...
                    change = (
                        (market.price - avg_price) / avg_price if avg_price > 0 else 0
                    )
