    ...
    async def _test():
        ...
        try:
            ...
            for market in markets[:5]:  # Check first 5
                ...
                assert (
                    market.last_price_cents > 0
                ), f"Market {market.market_id} has invalid price: {market.last_price_cents}"
                assert (
                    0 <= market.last_price_cents <= 10000
                ), f"Market {market.market_id} price out of range: {market.last_price_cents}"

                print(f"  Market: {market.market_id}")
                print(
                    f"    Price: ${market.price:.4f} ({market.last_price_cents} cents)"
                )
        ...
        try:
            ...
            for market in markets:
                ...
                if market.close_ts < now:
                    stale_count += 1
                    print(
                        f"⚠️  Market {market.market_id} has expired (close_ts: {market.close_ts})"
                    )
                else:
                    time_to_close = (market.close_ts - now) / 3600  # hours
                    print(
                        f"✅ Market {market.market_id} closes in {time_to_close:.1f} hours"
                    )
            ...
            assert (
                active_percent >= 50
            ), f"Too many stale markets: only {active_percent:.0f}% are active"
...
def test_price_variation_across_markets(config):
    ...
    async def _test():
        ...
        try:
            ...
            assert (
                diversity_ratio >= 0.3
            ), f"Prices lack diversity ({diversity_ratio*100:.0f}% unique). Possible stale data."
            ...
            print(
                f"   Markets at default price (5000 cents): {default_price_count} ({default_ratio*100:.0f}%)"
            )

            assert (
                default_ratio < 0.5
            ), f"Too many markets at default price ({default_ratio*100:.0f}%)"
    ...
    async def _test():
        ...
        try:
            ...
            for market in markets[:5]:
                ...
                assert (
                    abs(price_float - expected_float) < 0.0001
                ), f"Price conversion error: {price_float} != {expected_float}"

                print(
                    f"   ✅ {market.market_id}: {market.last_price_cents} → ${price_float:.4f}"
                )
