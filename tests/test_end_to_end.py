...
class TestEndToEnd:
    ...
    def config(self):
        ...
        config.MAX_DAILY_LOSS_PCT = (
            0.01  # 1% limit to ensure simulated losses trigger halt
        )
    ...
    def test_successful_profitable_trade(
        self, config, spike_detector, position_manager
    ):
        """
        ...
        async def _test():
            ...
            market = self.create_synthetic_market(
                market_id="PROFIT-TEST-001", price=0.30, volume=100.0
            )
            ...
            # Close position
            close_result = position_manager.close_position(order_id, exit_price)
    ...
    def test_stop_loss_trigger(self, config, spike_detector, position_manager):
        ...
        async def _test():
            ...
            # Create market
            market = self.create_synthetic_market(market_id="STOP-LOSS-001", price=0.40)

            # Build price history and detect spike
            base_time = datetime.now()
            for i in range(20):
                spike_detector.add_price(
                    market.market_id, 0.40, base_time - timedelta(minutes=20 - i)
                )
            ...
            assert (
                "stop_loss" in reason or "stop loss" in reason.lower()
    ...
    def test_daily_loss_limit_halts_trading(self, config):
        ...
        async def _test():
            ...
            risk_manager = RiskManager(
                client=mock_client, config=config, fee_calculator=fee_calc
            )
            await risk_manager.initialize_daily(starting_balance=1000.0)

            assert (
                risk_manager.daily_loss_limit.max_daily_loss_pct == 0.01
            ), f"Config mismatch: {risk_manager.daily_loss_limit.max_daily_loss_pct}"
            print(
                f"   Risk Limit: {risk_manager.daily_loss_limit.max_daily_loss_pct:.1%}"
            )

            # Setup Position Manager with Risk Manager
            position_manager = PositionManager(
                platform="kalshi", config=config, risk_manager=risk_manager
            )
            ...
            for i in range(5):
                ...
                final_pnl = position_manager.close_position(
                    position["id"], exit_price=0.40  # Adverse move
                )
                ...
                print(
                    f"   Trade {i+1}: ${final_pnl.get('net_pnl', final_pnl.get('pnl', 0)):.2f} | Total: ${total_loss:.2f} | Loss: {status.get('loss_pct', 0):.2%}"
                )
            ...
            assert (
                total_loss < -15.0
            ), f"Simulated loss {total_loss} insufficient to trigger limit"

            print(f"\n💸 Total losses: ${total_loss:.2f}")

            # Verify Risk Manager actually halts trading
            can_trade = await risk_manager.can_trade_pre_submission(
                Mock(change_pct=0.05)
            )
            assert (
                can_trade.passed is False
            ), "Risk Manager should block trading after limit hit"
    ...
    def test_no_spikes_no_trades(self, config, spike_detector):
        ...
        async def _test():
            ...
            # Create market
            market = self.create_synthetic_market(
                market_id="STABLE-MARKET-001", price=0.50
            )
    ...
    def test_spike_detection_with_insufficient_history(self, config, spike_detector):
        ...
        async def _test():
            ...
            market = self.create_synthetic_market(
                market_id="NEW-MARKET-001", price=0.60
            )
            ...
            for i in range(5):
                spike_detector.add_price(
                    market.market_id, 0.60, base_time - timedelta(minutes=5 - i)
                )
            ...
            if history_length < min_history_length:
                assert (
                    len(spikes) == 0
                ), "Should not detect spikes with insufficient history"
            ...
            for i in range(min_history_length - 5):
                spike_detector.add_price(
                    market.market_id, 0.60, base_time - timedelta(minutes=25 - i)
                )
