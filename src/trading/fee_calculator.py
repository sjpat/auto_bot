...
class FeeCalculator:
    ...
    def kalshi_fee(
        self, contracts: int, price: float, fee_type: str = "taker"
    ) -> float:
        """
    ...
    def entry_cost(
        self, contracts: int, entry_price: float, fee_type: str = "taker"
    ) -> Dict[str, float]:
        """
    ...
    def exit_revenue(
        self, contracts: int, exit_price: float, fee_type: str = "taker"
    ) -> Dict[str, float]:
        """
    ...
    def breakeven_price_move_percent(
        ...
        move_pct = (
            (breakeven_price - entry_price) / entry_price if entry_price > 0 else 0
        )
        return move_pct
    ...
    def fee_impact_analysis(
        self, entry_price: float, exit_price: float, contracts: int
    ) -> Dict[str, Any]:
        """
        ...
        entry_fee_pct = (
            (entry_fee / (entry_price * contracts)) * 100 if entry_price > 0 else 0
        )
        exit_fee_pct = (
            (exit_fee / (exit_price * contracts)) * 100 if exit_price > 0 else 0
        )
        ...
        return {
            ...
            "fee_impact_pct": (
                (total_fees / pnl.gross_profit * 100) if pnl.gross_profit > 0 else 0
            ),
