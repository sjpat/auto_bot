...
class PaperTradingClient:
    ...
    def __init__(
        ...
        self.logger.info(
            f"📈 Slippage: {'ENABLED' if simulate_slippage else 'DISABLED'}"
        )
        self.logger.info("=" * 80)
    ...
    async def get_markets(
        self, status: str = "open", limit: int = 1000, filter_untradeable: bool = True
    ) -> List[Market]:
    ...
    def _save_trade_to_history(self, position: PaperPosition):
        ...
        trade_record = {
            ...
            "gross_pnl": (position.exit_price - position.entry_price)
            * position.quantity,
    ...
    async def start_streaming(self, callback):
        """Pass through to real client's streaming method."""
        if hasattr(self.real_client, "start_streaming"):
            await self.real_client.start_streaming(callback)
        else:
            self.logger.error("Streaming not supported by the underlying client.")
            raise NotImplementedError(
                "Streaming not supported by the underlying client."
            )

    async def stop_streaming(self):
        """Pass through to real client's streaming stop method."""
        if hasattr(self.real_client, "stop_streaming"):
            await self.real_client.stop_streaming()
