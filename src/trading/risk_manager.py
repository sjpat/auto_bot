...
class DailyLossLimit:
    ...
    async def check_daily_loss_limit(self, current_balance: float) -> Dict[str, Any]:
        """
...
class SlippageMonitor:
    ...
    def should_reject_fill(
        self, requested_price: float, actual_fill_price: float, side: str
    ) -> bool:
        """
...
class SettlementTracker:
    ...
    def __init__(self):
        ...
        self.pending_settlements: Dict[str, datetime] = (
            {}
        )  # position_id -> settlement_time

    def track_settlement(
        self, position_id: str, exit_amount: float, exit_time: Optional[datetime] = None
    ):
        """
    ...
    def get_settlement_status(self) -> Dict[str, Any]:
        ...
        now = datetime.now()
        pending_count = sum(1 for st in self.pending_settlements.values() if st > now)

        return {
            "pending_settlements": pending_count,
            "total_pending_amount": sum(
                v
                for k, v in self.settled_positions.items()
                if k in self.pending_settlements and self.pending_settlements[k] > now
            ),
            "next_settlement": (
                min(self.pending_settlements.values())
            ...
            "settled_positions": len(
                [
                    k
                    for k in self.settled_positions.keys()
                    if k not in self.pending_settlements
                    or self.pending_settlements[k] <= now
                ]
            ),
        }
...
class RiskManager:
    ...
    async def track_exit(self, position_id: str, exit_amount: float):
        """
    ...
    def handle_api_error(self, status_code: int, error: Exception) -> bool:
        ...
        if is_suspended:
            self.logger.critical("🔴 ACCOUNT SUSPENSION DETECTED - TRADING HALTED")
