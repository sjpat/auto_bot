...
class Config:
    ...
    KALSHI_DEMO_API_KEY = os.getenv("KALSHI_DEMO_API_KEY")
    KALSHI_DEMO_PRIVATE_KEY_PATH = os.getenv("KALSHI_DEMO_PRIVATE_KEY_PATH")
    KALSHI_DEMO = os.getenv("KALSHI_DEMO", "false").lower() == "true"

    # Fee Settings
    TAKER_FEE_MULTIPLIER: float = float(os.getenv("TAKER_FEE_MULTIPLIER", "0.07"))
    MAKER_FEE_MULTIPLIER: float = float(os.getenv("MAKER_FEE_MULTIPLIER", "0.0175"))
    ...
    MIN_CONFIDENCE_MISPRICING: float = float(
        os.getenv("MIN_CONFIDENCE_MISPRICING", "0.60")
    )
    MISPRICING_MAX_HOLDING_TIME: int = int(
        os.getenv("MISPRICING_MAX_HOLDING_TIME", "14400")
    )  # 4 hours
    MISPRICING_HISTORY_SIZE: int = int(os.getenv("MISPRICING_HISTORY_SIZE", "50"))

    # ===== Strategy Selection =====
    ENABLE_SPIKE_STRATEGY: bool = (
        os.getenv("ENABLE_SPIKE_STRATEGY", "true").lower() == "true"
    )
    ENABLE_MISPRICING_STRATEGY: bool = (
        os.getenv("ENABLE_MISPRICING_STRATEGY", "true").lower() == "true"
    )
    ENABLE_MOMENTUM_STRATEGY: bool = (
        os.getenv("ENABLE_MOMENTUM_STRATEGY", "false").lower() == "true"
    )
    ENABLE_VOLUME_STRATEGY: bool = (
        os.getenv("ENABLE_VOLUME_STRATEGY", "false").lower() == "true"
    )
    ...
    MOMENTUM_REVERSAL_MULTIPLIER: float = float(
        os.getenv("MOMENTUM_REVERSAL_MULTIPLIER", "0.5")
    )

    # ===== Volume Strategy Parameters =====
    VOLUME_SPIKE_THRESHOLD: float = float(
        os.getenv("VOLUME_SPIKE_THRESHOLD", "3.0")
    )  # 3x average
    ...
    MIN_LIQUIDITY_USD: float = float(os.getenv("MIN_LIQUIDITY_REQUIREMENT", "200.0"))
    ...
    MAX_ORDER_LATENCY_SECONDS: int = int(os.getenv("MAX_ORDER_LATENCY_SECONDS", "5"))
    ...
    PRICE_UPDATE_INTERVAL: float = float(
        os.getenv("PRICE_UPDATE_INTERVAL", "0.5")  # seconds
    )
    ...
    PAPER_HISTORY_FILE = os.getenv(
        "PAPER_HISTORY_FILE", "logs/paper_trading_history.json"
    )

    def __post_init__(self):
        """Validate configuration after initialization"""
        if isinstance(self.TARGET_EVENT_KEYWORDS, str):
            self.TARGET_EVENT_KEYWORDS = [
                kw.strip() for kw in self.TARGET_EVENT_KEYWORDS.split(",") if kw.strip()
            ]
        ...
        if self.platform == "polymarket":
            ...
            if self.KALSHI_DEMO:
                if not self.KALSHI_DEMO_API_KEY:
                    raise ValueError("KALSHI_DEMO_API_KEY required for Kalshi Demo")
                if not self.KALSHI_DEMO_PRIVATE_KEY_PATH:
                    raise ValueError(
                        "KALSHI_DEMO_PRIVATE_KEY_PATH required for Kalshi Demo"
                    )
                if not os.path.exists(self.KALSHI_DEMO_PRIVATE_KEY_PATH):
                    raise ValueError(
                        "Kalshi demo private key file not found: "
                        f"{self.KALSHI_DEMO_PRIVATE_KEY_PATH}"
                    )
            else:
