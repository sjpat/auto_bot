...
class MomentumStrategy(BaseStrategy):
    ...
    def __init__(self, config: Dict[str, Any]):
        ...
        self.momentum_window = config.get(
            "MOMENTUM_WINDOW", 6
        )  # Number of updates to look back
        ...
        self.holding_time_limit = config.get(
            "HOLDING_TIME_LIMIT", 1800
        )  # 30 mins max for momentum
    ...
    def generate_exit_signals(
        ...
        for position in positions:
            ...
            if position.unrealized_pnl >= self.target_profit_usd:
                signals.append(
                    self._create_exit(position, market.yes_price, "profit_target")
                )
            elif position.unrealized_pnl <= self.target_loss_usd:
                signals.append(
                    self._create_exit(position, market.yes_price, "stop_loss")
                )
            elif position.holding_time_seconds >= self.holding_time_limit:
                signals.append(
                    self._create_exit(position, market.yes_price, "time_limit")
                )
                ...
                if len(history) > self.momentum_window:
                    ...
                    if getattr(position, "side", "buy") == "buy":
                        if roc < -self.momentum_threshold * self.reversal_multiplier:
                            signals.append(
                                self._create_exit(
                                    position, market.yes_price, "trend_reversal"
                                )
                            )
