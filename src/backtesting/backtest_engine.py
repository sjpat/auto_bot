...
class BacktestConfig:
    ...
    # Account settings
    starting_balance: float = float(os.getenv("PAPER_STARTING_BALANCE", "10000.0"))

    # Position sizing
    TRADE_UNIT: int = int(os.getenv("TRADE_UNIT", "100"))
    MAX_CONCURRENT_TRADES: int = int(os.getenv("MAX_CONCURRENT_TRADES", "3"))

    # Spike detection
    SPIKE_THRESHOLD: float = float(os.getenv("SPIKE_THRESHOLD", "0.05"))

    # Exit targets
    TARGET_PROFIT_USD: float = float(os.getenv("TARGET_PROFIT_USD", "2.50"))
    TARGET_LOSS_USD: float = float(os.getenv("TARGET_LOSS_USD", "-1.50"))

    # Slippage
    MAX_SLIPPAGE_TOLERANCE: float = float(os.getenv("MAX_SLIPPAGE_TOLERANCE", "0.025"))

    # Market filtering
    MIN_LIQUIDITY_USD: float = float(os.getenv("MIN_LIQUIDITY_USD", "500.0"))
    MAX_SPREAD_PCT: float = float(os.getenv("MAX_SPREAD_PCT", "0.30"))
    MIN_PRICE_HISTORY: int = int(os.getenv("MIN_PRICE_HISTORY", "20"))
    MAX_SUSPICIOUS_SPIKE_PCT: float = float(
        os.getenv("MAX_SUSPICIOUS_SPIKE_PCT", "0.30")
    )
    MIN_TIME_TO_EXPIRY_HOURS: float = (
        float(os.getenv("MIN_TIME_TO_EXPIRY_SECONDS", "1800")) / 3600
    ...
    TRAILING_STOP_ACTIVATION_USD: float = float(
        os.getenv("TRAILING_STOP_ACTIVATION_USD", "5.00")
    )
    TRAILING_STOP_DISTANCE_USD: float = float(
        os.getenv("TRAILING_STOP_DISTANCE_USD", "2.50")
    )
...
class Trade:
    ...
    def holding_time_seconds(self) -> float:
        return (
            (self._current_timestamp - self.entry_timestamp).total_seconds()
...
class BacktestMarketAdapter:
    ...
    def is_liquid(
        self, min_liquidity: float = None, min_liquidity_usd: float = None
    ) -> bool:
    ...
    def time_to_close_seconds(self) -> float:
        ...
        if (
            hasattr(self.price_point, "expiry_timestamp")
            and self.price_point.expiry_timestamp
        ):
...
class BacktestEngine:
    ...
    async def run_backtest(
        ...
        for timestamp in sorted_timestamps:
            ...
            for market_id, price_points in historical_data.items():
                ...
                spike_signal = await self._detect_spike(
                    [market_adapter], price_point, results
                )
    ...
    async def _check_market_filters(
        ...
        if hasattr(price_point, "max_price") and hasattr(price_point, "min_price"):
            price_range = (
                abs(price_point.max_price - price_point.min_price)
                / price_point.min_price
            )
    ...
    async def _detect_spike(
        ...
        spike_signals = self.strategy_manager.generate_entry_signals(markets)
    ...
    async def _process_trade_entry(
        ...
        mock_spike = MockSpike(
            change_pct=spike_signal.get("spike_magnitude", 0.0), market_id=market_id
        )

        # Check risk manager
        risk_check = await self.risk_manager.can_trade_pre_submission(spike=mock_spike)
        ...
        side = (
            OrderSide.BUY if spike_signal.get("direction") == "up" else OrderSide.SELL
        )

        # Calculate entry cost
        entry_cost = price_point.yes_price * self.config.TRADE_UNIT
        entry_fees = self.fee_calculator.kalshi_fee(
            self.config.TRADE_UNIT, price_point.yes_price
        )
        total_cost = entry_cost + entry_fees

        # Check correlation risk
        corr_passed, corr_reason = self.correlation_manager.check_exposure(
            market_id, entry_cost
        )
        if not corr_passed:
            results.signals_rejected += 1
    ...
    async def _process_trade_exits(
        ...
        for market_id, trade in self.open_trades.items():
            ...
            unrealized_pnl = (
                current_price.yes_price - trade.entry_price
            ...
            if (
                hasattr(current_price, "expiry_timestamp")
                and current_price.expiry_timestamp
            ):
                time_to_expiry = (
                    current_price.expiry_timestamp - current_timestamp
                ).total_seconds() / 3600
            ...
            if not exit_triggered and self.config.USE_TRAILING_STOP:
                if trade.max_unrealized_pnl >= self.config.TRAILING_STOP_ACTIVATION_USD:
                    if unrealized_pnl <= (
                        ...
                        - self.config.TRAILING_STOP_DISTANCE_USD
                    ):
                        exit_triggered = True
            ...
            if exit_triggered:
                trades_to_close.append(
                    (trade, current_price.yes_price, current_timestamp, exit_reason)
                )
        ...
        if active_trades_for_strategy:
            ...
            for market_id in self.open_trades.keys():
                if market_id in historical_data:
                    ...
                    for point in historical_data[market_id]:
                        if point.timestamp == current_timestamp:
                            market_map[market_id] = BacktestMarketAdapter(
                                market_id, point
                            )
                            break

            # Get exit signals from strategy manager
            strategy_exits = self.strategy_manager.generate_exit_signals(
                active_trades_for_strategy, market_map
            )
                ...
                if trade and trade not in [t[0] for t in trades_to_close]:
                    trades_to_close.append(
                        (
                            ...
                            signal.price,
                            current_timestamp,
                            signal.metadata.get("reason", "strategy_exit"),
                        )
                    )
    ...
    def _calculate_results(self, results: BacktestResults) -> None:
        ...
        results.total_return_pct = (
            results.total_return_usd / results.starting_balance
        ) * 100
        ...
        logger.info(
            f"Results calculated: {results.total_trades} trades, {results.win_rate:.1f}% win rate"
        )
