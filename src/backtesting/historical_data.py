...
class HistoricalPricePoint:
    ...
    def from_dict(cls, data: dict):
        ...
        if "price" in data:
            data["yes_price"] = data.pop("price")
        if "yes_bid" in data:
            data["bid"] = data.pop("yes_bid")
        if "yes_ask" in data:
            data["ask"] = data.pop("yes_ask")
        if "volume" in data:
            data["volume_24h"] = data.pop("volume")
        if "liquidity" in data:
            data["liquidity_usd"] = data.pop("liquidity")
...
class HistoricalDataFetcher:
    ...
    async def fetch_market_history(
        ...
        try:
            ...
            history_data = await self.client.get_market_history(
                market_id=market_id, last_seen_ts=start_ts
            )
            ...
            self.logger.info(
                f"Fetched {len(price_points)} historical points for {market_id}"
            )
            return price_points
    ...
    def from_database(
        ...
        self.logger.info(
            f"Converted {len(backtest_data)} markets from DB to backtest format"
        )
    ...
    async def fetch_settled_markets(
        ...
        try:
            ...
            if len(markets) < 10:
                ...
                self.logger.info(
                    "Few settled markets found, also fetching closed markets..."
                )
                closed_markets = await self.client.get_markets(
            ...
            for market in markets:
                ...
                if volume >= min_volume:
                    ...
                    if category is None or (
                        hasattr(market, "category") and market.category == category
                    ):
    ...
    def _save_to_cache(
        self, cache_file: Path, price_points: List[HistoricalPricePoint]
    ):
    ...
    def _save_candlesticks_to_cache(
        self, cache_file: Path, candlesticks: List[Candlestick]
    ):
    ...
    async def build_backtest_dataset(
        ...
        """
        self.logger.info(
            f"Building backtest dataset from {start_date.date()} to {end_date.date()}"
        )

        # Fetch settled markets in date range
        markets = await self.fetch_settled_markets(
            start_date=start_date, end_date=end_date, min_volume=min_volume
        )
        ...
        for i, market in enumerate(markets, 1):
            ...
            history = await self.fetch_market_history(
                market_id=market.market_id, use_cache=True, market_expiry=expiry
            )

            if history:
                # Filter to date range
                filtered_history = [
                    p for p in history if start_date <= p.timestamp <= end_date
                ]
