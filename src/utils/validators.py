...
def validate_price(
    price: float, min_price: float = 0.0, max_price: float = 1.0
) -> bool:
    """
...
def validate_quantity(
    quantity: int, min_quantity: int = 1, max_quantity: int = 10000
) -> bool:
    """
...
def validate_balance(
    balance: float, required_amount: float, min_reserve: float = 0.0
) -> bool:
    """
...
def validate_order_params(
    side: str, quantity: int, price: Optional[float] = None, order_type: str = "limit"
) -> bool:
    """
    ...
    if order_type.lower() not in ["limit", "market"]:
        raise ValidationError(
            f"Invalid order type: {order_type} (must be 'limit' or 'market')"
        )
...
def validate_slippage(
    requested_price: float, actual_price: float, max_slippage_pct: float = 0.025
) -> bool:
    """
...
def validate_profit_target(
    entry_cost: float, target_profit: float, max_return_pct: float = 5.0
) -> bool:
    """
...
def validate_position_size(
    position_cost: float, account_balance: float, max_position_pct: float = 0.10
) -> bool:
    """
...
def validate_config(config: Dict[str, Any]) -> bool:
    ...
    if config["MAX_DAILY_LOSS_PCT"] <= 0 or config["MAX_DAILY_LOSS_PCT"] > 1:
        raise ValidationError(
            f"Invalid MAX_DAILY_LOSS_PCT: {config['MAX_DAILY_LOSS_PCT']}"
        )

    if config["MAX_SLIPPAGE_TOLERANCE"] <= 0 or config["MAX_SLIPPAGE_TOLERANCE"] > 1:
        raise ValidationError(
            f"Invalid MAX_SLIPPAGE_TOLERANCE: {config['MAX_SLIPPAGE_TOLERANCE']}"
        )
